...
type RegraExpedicaoRowProps = {
  regra: RegraExpedicao;
...
export function RegraExpedicaoRow({
  regra,
  tempoPreviewSeg,
  onToggleAtivo,
  onDuplicar,
  onExcluir,
}: RegraExpedicaoRowProps) {
  const editHref = regrasProdutividadeEditPath('separacao', regra.id);
    ...
    >
      ...
      <td className={cn(compactTableCellClassName, 'hidden sm:table-cell')}>
        ...
        <p className="text-[10px] text-muted-foreground">
          {PREVIEW_QTD_ITENS} itens · {PREVIEW_QTD_ENDERECOS} endereços ·{' '}
          {PREVIEW_QTD_ITENS_SEM_ENDERECO} s/ endereço
        </p>
      </td>
