...
import {
  ...
  DEFAULT_REGRA_CARREGAMENTO_FORM,

  regraCarregamentoFormSchema,

  type RegraCarregamentoForm,
...
type UseRegraCarregamentoFormOptions = {
...
export function useRegraCarregamentoForm({ regraId }: UseRegraCarregamentoFormOptions = {}) {
  ...
  const form = useForm<RegraCarregamentoForm>({

    resolver: zodResolver(regraCarregamentoFormSchema),

    defaultValues: DEFAULT_REGRA_CARREGAMENTO_FORM,
  ...
  const [simQtdPaletes, setSimQtdPaletes] = useState(8);

  const [simQtdClientes, setSimQtdClientes] = useState(3);

  const [simQtdTabelas, setSimQtdTabelas] = useState(2);
  ...
  useEffect(() => {
    ...
    async function carregarRegra() {
      ...
      try {
        ...
        const parametros = extractParametrosCarregamento(

          item.parametros as Parameters<typeof extractParametrosCarregamento>[0],
  ...
  const onSubmit = form.handleSubmit(async (data: RegraCarregamentoForm) => {
    ...
    try {
      ...
      const parametros = extractParametrosCarregamento(data);
      ...
      router.push(regrasProdutividadeListaPath('carregamento'));
