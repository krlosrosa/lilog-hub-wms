...
type RegraCarregamentoRowProps = {
  regra: RegraCarregamento;
...
export function RegraCarregamentoRow({
  regra,
  tempoPreviewSeg,
  onToggleAtivo,
  onDuplicar,
  onExcluir,
}: RegraCarregamentoRowProps) {
  const editHref = regrasProdutividadeEditPath('carregamento', regra.id);
    ...
    >
      ...
      <td className={cn(compactTableCellClassName, 'hidden sm:table-cell')}>
        ...
        <p className="text-[10px] text-muted-foreground">
          {PREVIEW_QTD_PALETES} paletes · {PREVIEW_QTD_CLIENTES} clientes · {PREVIEW_QTD_TABELAS}{' '}
          tabelas
        </p>
      </td>
