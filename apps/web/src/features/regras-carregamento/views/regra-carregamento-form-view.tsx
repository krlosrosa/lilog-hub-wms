...
import { ParametrosCarregamentoPanel } from '@/features/regras-carregamento/components/parametros-carregamento-panel';
import { TempoCarregamentoPreview } from '@/features/regras-carregamento/components/tempo-carregamento-preview';
import { regrasProdutividadeListaPath } from '@/features/config-operacional/lib/regras-produtividade-paths';
import { useRegraCarregamentoForm } from '@/features/regras-carregamento/hooks/use-regra-carregamento-form';

type RegraCarregamentoFormViewProps = {
  regraId?: string;
};

export function RegraCarregamentoFormView({ regraId }: RegraCarregamentoFormViewProps) {
  const {
    form,
    isEditing,
    isLoading,
    ...
    simQtdTabelas,
    setSimQtdTabelas,
  } = useRegraCarregamentoForm({ regraId });
        ...
        <main className="flex min-h-dvh items-center justify-center px-margin-mobile py-12 md:px-margin-desktop">
          <div className="text-center">
            ...
            <Button asChild variant="link" className="mt-2">
              <Link href={regrasProdutividadeListaPath('carregamento')}>Voltar à lista</Link>
            </Button>
          </div>
        </main>
      </SidebarMain>
        ...
        <div className="mx-auto max-w-container">
          <div className="mb-6">
            <Button
              ...
              asChild
            >
              <Link href={regrasProdutividadeListaPath('carregamento')}>
            ...
            <form onSubmit={onSubmit}>
              <div className="grid gap-5 lg:grid-cols-[1fr_320px] lg:items-start">
                <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <input
                          id="nome"
                          {...register('nome')}
                          placeholder="Ex.: Perfil Alfa Carregamento"
                  ...
                  <CollapsiblePanelSection icon={Clock} title="Tempos de carga">
                    <ParametrosCarregamentoPanel
                      register={register}
                      errors={errors}
                      variant="carga"
                    />
                  ...
                  <CollapsiblePanelSection icon={Truck} title="Operações na doca">
                    <ParametrosCarregamentoPanel
                      register={register}
                      errors={errors}
                      variant="doca"
                  ...
                  <CollapsiblePanelSection icon={Clock} title="Gordura da minuta">
                    <p className={cn(sectionLabelClassName, 'mb-3 normal-case tracking-normal')}>
                      Margem de segurança no início e fim da execução da minuta.
                    </p>
                    <ParametrosCarregamentoPanel
                      register={register}
                      errors={errors}
                ...
                <div className="lg:sticky lg:top-6">
                  <TempoCarregamentoPreview
                    params={watchedValues}
                    qtdPaletes={simQtdPaletes}
                    qtdClientes={simQtdClientes}
                    qtdTabelas={simQtdTabelas}
                    onQtdPaletesChange={setSimQtdPaletes}
                    onQtdClientesChange={setSimQtdClientes}
                    onQtdTabelasChange={setSimQtdTabelas}
                  />
                </div>
              </div>
