...
import { regrasPausasEditPath } from '@/features/regras-pausas/lib/regras-pausas-paths';
...
export function RegraPausaRow({
  regra,
  onToggleAtivo,
  onDuplicar,
  onExcluir,
}: RegraPausaRowProps) {
  const editHref = regrasPausasEditPath(regra.tipo as TipoPausaRegra, regra.id);
    ...
    >
      ...
      <td className={cn(compactTableCellClassName, 'hidden sm:table-cell')}>
        <p className="text-[11px] font-medium text-foreground">
          {TIPO_PAUSA_REGRA_LABELS[regra.tipo]}
        </p>
      </td>

      <td className={cn(compactTableCellClassName, 'hidden md:table-cell')}>
        <p className="text-[11px] text-foreground">
          {regra.intervaloTrabalhoMinutos > 0
            ? `A cada ${formatIntervaloTrabalho(regra.intervaloTrabalhoMinutos)}`
            : 'Sem alerta automático'}
        </p>
        <p className="text-[10px] text-muted-foreground">
          Pausa de {regra.duracaoPausaMinutos} min
        </p>
      </td>
