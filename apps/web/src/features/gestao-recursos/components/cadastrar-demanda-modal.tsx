...
export function CadastrarDemandaModal({
  ...
  const [buscaPalete, setBuscaPalete] = useState('');
  ...
  const paleteInputRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    if (!open) return;
    const timer = window.setTimeout(() => paleteInputRef.current?.focus(), 100);
    ...
    async (identificador: string) => {
      ...
      if (!mapa) {
        try {
          ...
          toast.error('Não foi possível buscar o palete.');
          return;
        }
      }

      if (!mapa) {
        toast.error('Palete/mapa não encontrado com esse ID.');
        return;
      }

      setSelectedMapaIds((prev) => {
        if (prev.has(mapa!.id)) {
          toast.info('Este palete já está selecionado.');
          return prev;
        }
        const next = new Set(prev);
        next.add(mapa!.id);
        toast.success(`Palete ${mapa!.microUuid} adicionado.`);
        return next;
      });
      cacheMapaSelecionado(mapa);
      setBuscaPalete('');
  ...
  const handlePaleteKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key !== 'Enter') return;
    event.preventDefault();
    void adicionarMapaPorIdentificador(buscaPalete);
  };
    ...
    <Dialog open={open} onOpenChange={(next) => !next && onClose()}>
      <DialogContent
        className={cn(
          ...
          isCarregamento
            ? 'max-h-[min(88vh,680px)] sm:max-w-xl'
            : 'max-h-[min(92vh,820px)] sm:max-w-4xl',
        )}
      >
        <div className="shrink-0 space-y-3 border-b border-outline-variant px-5 pb-4 pt-5">
            ...
            {isCarregamento ? (
              <>
                <label className="space-y-1">
                  ...
                  <select
                    value={docaId}
                    disabled={isLoadingDocas || isSubmitting}
              ...
              <ProcessoMapaCombobox
                ...
                disabled={isLoadingMapas}
              />
            )}
          </div>

          {deveExibirAlertaPausa(selectedOperator) ? (
            <OperadorProximaPausaResumo operator={selectedOperator} />
          ) : null}
          ...
          {isCarregamento ? (
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    ref={paleteInputRef}
                    value={buscaPalete}
                    onChange={(event) => setBuscaPalete(event.target.value)}
                    onKeyDown={handlePaleteKeyDown}
                    placeholder="Escaneie ou digite o ID do palete..."
                    className={cn(fieldClassName, 'pl-8')}
                    disabled={isLoadingMapas}
                  />
                </div>
              ...
              {mapasSelecionados.length > 0 ? (
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between gap-2">
                    <span className={labelClassName}>Selecionados</span>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {mapasSelecionados.map((mapa) => (
                      <span
                        key={mapa.id}
                        className="inline-flex max-w-full items-center gap-1 rounded-full border border-primary/30 bg-primary/5 py-0.5 pl-2 pr-1 text-[11px] text-foreground"
                      >
                        <span className="truncate font-mono">{mapa.microUuid}</span>
                        <button
                          type="button"
                          className="rounded-full p-0.5 text-muted-foreground hover:bg-primary/10 hover:text-foreground"
                          onClick={() => removerMapa(mapa.id)}
                          aria-label={`Remover ${mapa.microUuid}`}
                        >
                          <X className="size-3" />
                        </button>
                      </span>
              ...
              <div className="space-y-1.5">
                ...
                <div className="rounded-md border border-outline-variant bg-muted/15 p-1.5">
                  {isLoadingMapas ? (
                    ...
                    <ul className="max-h-52 space-y-1 overflow-y-auto pr-0.5">
                      {mapas.map((mapa) => renderMapaDisponivel(mapa, true))}
                    </ul>
                  )}
                </div>
              </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <span className={labelClassName}>Paletes selecionados</span>
                <div className="relative">
                  <Search className="pointer-events-none absolute left-2.5 top-1/2 size-3.5 -translate-y-1/2 text-muted-foreground" />
                  <input
                    ref={paleteInputRef}
                    ...
                    onChange={(event) => setBuscaPalete(event.target.value)}
                    onKeyDown={handlePaleteKeyDown}
                    placeholder="Buscar palete por ID (QR)..."
                    className={cn(fieldClassName, 'pl-8')}
                    disabled={isLoadingMapas}
                  />
                </div>
                <div className="max-h-72 overflow-y-auto rounded-md border border-outline-variant bg-muted/15 p-1.5">
                  {mapasSelecionados.length === 0 ? (
                    <p className="px-2 py-6 text-center text-caption text-muted-foreground">
                      Escaneie ou selecione mapas.
                    </p>
                  ) : (
                      ...
                      {mapasSelecionados.map((mapa) => (
                        ...
                        >
                          ...
                          <Button
                            ...
                            onClick={() => removerMapa(mapa.id)}
                            aria-label={`Remover palete ${mapa.microUuid}`}
                          >
                            <X className="size-3" />
                          </Button>
                        </li>
              ...
              <div className="space-y-1.5">
                <span className={labelClassName}>Mapas disponíveis</span>
                <div className="max-h-96 overflow-y-auto rounded-md border border-outline-variant bg-muted/15 p-1.5">
                  {isLoadingMapas ? (
                    ...
                    <ul className="space-y-1">
                      {mapas.map((mapa) => renderMapaDisponivel(mapa))}
                    </ul>
                  )}
                </div>
              </div>
