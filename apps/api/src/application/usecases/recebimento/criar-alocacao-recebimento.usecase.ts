...
export type CriarAlocacaoRecebimentoUseCaseInput = {
...
@Injectable()
export class CriarAlocacaoRecebimentoUseCase {
  constructor(
  ...
  async execute(
    input: CriarAlocacaoUseCaseInput,
    ...
    if (sessao.status !== 'aberta') {
      throw new BadRequestException('A sessão precisa estar aberta para realizar atribuições');
    }
    ...
    try {
      const alocacao = await this.recebimentoAlocacaoRepository.criar({
...
type CriarAlocacaoUseCaseInput = CriarAlocacaoRecebimentoUseCaseInput;
