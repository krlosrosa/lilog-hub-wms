...
import {
  findEnderecoByCentroAndCodigoDb,
  findEnderecoByIdDb,
...
export class EnderecoService implements IEnderecoRepository {
  ...
  getKpi(filter?: { centroId?: string; unidadeId?: string }) {
  ...
  findByCentroAndCodigo(centroId: string, enderecoMascarado: string) {
    return findEnderecoByCentroAndCodigoDb(
      this.db,
      centroId,
      enderecoMascarado,
    );
