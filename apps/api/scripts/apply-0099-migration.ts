...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0099_itens_armazenagem_status_saldo.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0099 already applied');
    process.exit(0);
  }
  ...
  const columnExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'armazenagem'
        AND table_name = 'itens_armazenagem'
        AND column_name = 'status_saldo'
    ) AS exists
  `;
  ...
  console.log(
    alreadyMigrated
      ? 'status_saldo column already in place; recorded migration 0099'
