...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0095_contagem_saldo_endereco_ref.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0095 already applied');
    process.exit(0);
  }
  ...
  const columnExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'estoque'
        AND table_name = 'contagens'
        AND column_name = 'saldo_endereco_id'
    ) AS exists
  `;
  ...
  console.log(
    columnExists[0]?.exists
      ? 'Columns already exist; recorded migration 0095'
      : 'Migration 0095 applied successfully',
  );
} catch (error) {
