...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0094_armazem_layout.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0094 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'estoque'
        AND table_name = 'armazem_layouts'
    ) AS exists
  `;
  ...
  console.log(
    alreadyApplied
      ? 'Schema already present; recorded migration 0094'
      : 'Migration 0094 applied successfully',
  );
} catch (error) {
