...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0090_motivos_bloqueio_saldo.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0090 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'estoque'
        AND table_name = 'motivos_bloqueio_saldo'
    ) AS exists
  `;
  ...
  console.log(
    alreadyApplied
      ? 'Schema already present; recorded migration 0090'
