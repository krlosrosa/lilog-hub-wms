...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0082_saldo_endereco_reserva_estoque.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0082 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'estoque'
        AND table_name = 'saldos_endereco'
    ) AS exists
  `;
  ...
  if (tableExists[0]?.exists) {
    ...
    `;
    console.log('Tables already exist; recorded migration 0082');
    process.exit(0);
  }
  ...
  console.log('Migration 0082 applied successfully');
} catch (error) {
