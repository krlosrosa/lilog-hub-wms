...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0101_usuarios_terceiros.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0101 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'auth'
        AND table_name = 'usuarios_terceiros'
    ) AS exists
  `;
  ...
  console.log(
    alreadyApplied
      ? 'Schema already present; recorded migration 0101'
      : 'Migration 0101 applied successfully',
  );
} catch (error) {
