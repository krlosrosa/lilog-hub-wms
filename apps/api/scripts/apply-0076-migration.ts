...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0076_cnc_gestao_completa.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0076 already applied');
    process.exit(0);
  }
  ...
  const tableCheck = await sql<{ table_name: string }[]>`
    ...
    WHERE table_schema = 'cnc'
      AND table_name IN ('cnc_eventos', 'cnc_tratativas')
  `;
  ...
  if (tableCheck.length === 2) {
    ...
    `;
    console.log('Schema already on CNC gestao completa; recorded migration 0076');
    process.exit(0);
  }
