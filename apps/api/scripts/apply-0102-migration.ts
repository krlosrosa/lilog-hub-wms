...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0102_inventario_divergencias.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0102 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'estoque'
        AND table_name = 'inventario_divergencias'
    ) AS exists
  `;
  ...
  console.log(
    alreadyApplied
      ? 'Schema already present; recorded migration 0102'
      : 'Migration 0102 applied successfully',
  );
} catch (error) {
