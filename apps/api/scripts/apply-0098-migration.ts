...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0098_add_endereco_tipo_aereo.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0098 already applied');
    process.exit(0);
  }

  const enumValueExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      SELECT 1
      FROM pg_enum e
      JOIN pg_type t ON e.enumtypid = t.oid
      WHERE t.typname = 'endereco_tipo_type'
        AND e.enumlabel = 'aereo'
    ) AS exists
  `;
  ...
  console.log(
    enumValueExists[0]?.exists
      ? 'Enum value already exists; recorded migration 0098'
      : 'Migration 0098 applied successfully',
  );
} catch (error) {
