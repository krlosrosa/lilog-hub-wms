...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0083_produto_grupo.sql'
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0083 already applied');
    process.exit(0);
  }
  ...
  console.log(
    columnExists[0]?.exists
      ? 'Column already exists; recorded migration 0083'
      : 'Migration 0083 applied successfully'
  );
} catch (error) {
