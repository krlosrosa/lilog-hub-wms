...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0104_portal_notificacoes.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0104 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'cobranca_transportadora'
        AND table_name = 'portal_notificacoes'
    ) AS exists
  `;
  ...
  console.log(
    alreadyApplied
      ? 'Schema already present; recorded migration 0104'
      : 'Migration 0104 applied successfully',
  );
} catch (error) {
