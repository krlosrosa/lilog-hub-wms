...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0086_regras_enderecamento_destino_rua.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0086 already applied');
    process.exit(0);
  }
  ...
  const columnExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'armazenagem'
        AND table_name = 'regras_enderecamento_destinos'
        AND column_name = 'rua'
    ) AS exists
  `;
  ...
  console.log(
    columnExists[0]?.exists
      ? 'Column rua already exists; recorded migration 0086'
      : 'Migration 0086 applied successfully',
  );
} catch (error) {
