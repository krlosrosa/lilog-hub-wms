...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0105_inventario_divergencia_recontagens.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0105 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'estoque'
        AND table_name = 'inventario_divergencia_recontagens'
    ) AS exists
  `;
  ...
  console.log(
    alreadyApplied
      ? 'Schema already present; recorded migration 0105'
      : 'Migration 0105 applied successfully',
  );
} catch (error) {
