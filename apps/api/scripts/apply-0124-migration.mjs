...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0124_pre_recebimento_termo_palete.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0124_pre_recebimento_termo_palete already applied.');
    process.exit(0);
  }
  ...
  const columnCheck = await sql`
    ...
    ) AS has_numero_termo_palete
  `;

  console.log('Migration 0124_pre_recebimento_termo_palete applied successfully.');
