...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0085_regras_processo.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0085 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'operacional'
        AND table_name = 'regras_processo'
    ) AS exists
  `;
  ...
  console.log(
    tableExists[0]?.exists
      ? 'Tables already exist; recorded migration 0085'
      : 'Migration 0085 applied successfully',
  );
} catch (error) {
