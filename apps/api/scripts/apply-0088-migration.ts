...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0088_drop_saldos_tratativa.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0088 already applied');
    process.exit(0);
  }

  const saldosTratativaExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      SELECT 1
  ...
  const saldosExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      SELECT 1
      FROM information_schema.tables
      WHERE table_schema = 'estoque'
  ...
  const alreadyDropped =
    !saldosTratativaExists[0]?.exists && !saldosExists[0]?.exists;
  ...
  console.log(
    alreadyDropped
      ? 'Tables already dropped; recorded migration 0088'
      : 'Migration 0088 applied successfully',
  );
} catch (error) {
