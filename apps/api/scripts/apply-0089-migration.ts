...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0089_drop_movement_records.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0089 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'estoque'
        AND table_name = 'movement_records'
    ) AS exists
  `;

  const alreadyDropped = !tableExists[0]?.exists;
  ...
  console.log(
    alreadyDropped
      ? 'Table already dropped; recorded migration 0089'
      : 'Migration 0089 applied successfully',
  );
} catch (error) {
