...
const migrationPath = resolve(
  __dirname,
  '../src/infra/db/providers/drizzle/config/migrations/0084_regras_enderecamento.sql',
);
...
try {
  ...
  if (existing.length > 0) {
    console.log('Migration 0084 already applied');
    process.exit(0);
  }
  ...
  const tableExists = await sql<{ exists: boolean }[]>`
    SELECT EXISTS (
      ...
      WHERE table_schema = 'armazenagem'
        AND table_name = 'regras_enderecamento'
    ) AS exists
  `;
  ...
  console.log(
    tableExists[0]?.exists
      ? 'Tables already exist; recorded migration 0084'
      : 'Migration 0084 applied successfully',
  );
} catch (error) {
