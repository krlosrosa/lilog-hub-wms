...
import type { CompanyCode } from '../types/devolucao.schema';
...
export function CompanyAvatarGroup({
  ...
  const unique = [...new Set(companies)].slice(0, 3) as CompanyCode[];
  ...
  return (
    <div
      ...
      aria-label={unique.map((c) => `${COMPANY_AVATAR[c].label} (${c})`).join(', ')}
    >
      <div className={cn('flex items-center', overlapClass)}>
        {unique.map((code, index) => {
          const config = COMPANY_AVATAR[code];
          return (
            <div
              key={code}
              className={cn(
