...
function PaletesResumoBadge({
  ...
  const divergente = recebidos !== undefined && recebidos !== esperados;
  const recebidosLabel = recebidos ?? '—';

  return (
    <div
      className={cn(
        'flex shrink-0 items-center gap-1 rounded-full border px-2 py-0.5',
        divergente
          ? 'border-warning/40 bg-warning-container/30'
          : 'border-on-primary-container/20 bg-on-primary-container/12'
      )}
      title={
        recebidos !== undefined
          ? `${recebidos} de ${esperados} paletes recebidos`
          : `Paletes esperados: ${esperados}`
      }
    >
      <PaleteIcon
        className={cn(
          'h-3.5 w-3.5',
          divergente ? 'text-on-warning-container' : 'text-on-primary-container/65'
        )}
      />
      <span className="font-mono text-[11px] font-bold leading-none tabular-nums">
        <span className={cn(divergente && 'text-on-warning-container')}>
          {recebidosLabel}
        </span>
        <span className="text-on-primary-container/40">/</span>
        <span className="text-on-primary-container/90">{esperados}</span>
      </span>
    </div>
  );
}
    ...
    <article className="relative overflow-hidden rounded-lg bg-primary-container px-3 py-2.5 text-on-primary-container">
      <div className="relative z-10 flex items-center gap-2.5">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-on-primary-container/10">
          <LogOut className="h-4 w-4 text-on-secondary-container" aria-hidden />
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 flex min-w-0 items-center gap-2">
            <p className="shrink-0 text-[10px] text-on-primary-container/70">Doca {dock}</p>
            <PaletesResumoBadge
              recebidos={paletesRecebidos}
              esperados={paletesEsperados}
            />
          </div>
        </div>
...
function SkuItemRow({
  ...
  const isReentrega = item.isReentrega === true;
    ...
    >
      ...
      <div className="min-w-0 flex-1 space-y-0.5">
        <div className="flex min-w-0 items-center justify-between gap-2">
          ...
          {!isConferido &&
            (isReentrega && item.quantidadeEsperada !== undefined ? (
              <span
                className="shrink-0 font-mono text-[10px] tabular-nums text-on-surface-variant"
                title={`Quantidade esperada: ${item.quantidadeEsperada} unidades`}
              >
                Esp.{' '}
        ...
        {(item.hasAvaria || item.hasDivergencia || isConferido || isReentrega) && (
          <div className="flex flex-wrap items-center gap-1 pt-0.5">
            ...
            {isReentrega && (
              <span className="inline-flex items-center gap-0.5 rounded-full border border-primary/30 bg-primary/10 px-1.5 py-px text-[10px] font-medium text-primary">
...
function getFilterLabel(activeFilter: SkuItemFilter | null): string {
  if (!activeFilter) return 'Itens da carga';
  return SKU_ITEM_FILTERS.find((f) => f.id === activeFilter)?.label ?? 'Itens da carga';
}
    ...
    <div className="page-enter flex flex-col">
      ...
      <section className="flex flex-col gap-2 px-margin-mobile pb-[calc(88px+env(safe-area-inset-bottom,0px))] pt-3">
        <ConferenciaResumoCard
          ...
          dock={dock}
          progress={progress}
          paletesEsperados={paletesEsperados}
        ...
        {!isEmpty && (
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="rounded-full bg-surface-container px-2.5 py-0.5 font-mono text-label-sm tabular-nums text-on-surface-variant">
              {items.length} de {progress.total}
            </span>
          </div>
        )}
        ...
        {isEmpty ? (
          <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
            ...
            <p className="text-headline-md font-semibold text-on-surface">Nenhum item encontrado</p>
            <p className="max-w-xs text-body-sm text-on-surface-variant">
              {activeFilter || search
                ? 'Ajuste o filtro ou a busca.'
                : 'Nenhum SKU nesta carga.'}
            </p>
          </div>
        ) : (
