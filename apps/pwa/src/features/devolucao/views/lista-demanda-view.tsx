...
function DevolucaoResumoCard({
  ...
  const stats = [
    ...
    { icon: LogOut, label: 'Capac.', value: `${patioStats.capacityPercent}%` },
  ] as const;
...
export function ListaDemandaView() {
  ...
  const { search, filteredDemands, patioStats, isLoading, isStale, isRefreshing } = state;
  ...
  const segregadaCount = useMemo(
    () => filteredDemands.filter((d) => d.cargaSegregada).length,
    [filteredDemands]
  );
  ...
  const displayedDemands = useMemo(() => {
    ...
    if (activeFilter === 'segregada') {
      return filteredDemands.filter((d) => d.cargaSegregada);
    }
          ...
          <div className="space-y-2.5 px-margin-mobile pb-3">
            ...
            {!isLoading && (
              ...
              >
                ...
                <FilterChip
                  label="Segregadas"
                  count={segregadaCount}
                  active={activeFilter === 'segregada'}
                  onClick={() => setActiveFilter('segregada')}
                />
              </div>
            )}
          </div>
          ...
          {!isLoading && !isFilteredEmpty && (
            <div className="flex items-center justify-between gap-2">
              <h2 className="flex items-center gap-1.5 text-label-md font-semibold text-on-surface">
                <LogOut className="h-4 w-4 text-secondary" aria-hidden />
                {activeFilter === 'priority'
                  ? 'Prioritárias'
                  : activeFilter === 'segregada'
                    ? 'Cargas segregadas'
            ...
            displayedDemands.map((demand) => (
              ...
              >
                <DemandCard
                  ...
                  cargaSegregada={demand.cargaSegregada}
                  pulse={demand.pulse}
