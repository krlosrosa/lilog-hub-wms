...
interface ChecklistV2ViewProps {
  demandId: string;
  viewOnly?: boolean;
}

export function ChecklistV2View({ demandId, viewOnly = false }: ChecklistV2ViewProps) {
  const navigate = useNavigate();
  const { checklist, saveChecklist, isComplete, isLoading } = useChecklistV2(demandId);
  ...
  async function handleRetomarConferencia() {
    try {
      ...
      if (isComplete) {
        await navigate({ to: '/recebimento-v2/$id/itens', params: { id: demandId } });
      }
    } catch (err) {
