...
import { Route as DetalheItemRoute } from '@/routes/recebimento-v2/$id.index';

import { AdicionarProdutoSheetV2 } from '../components/adicionar-produto-sheet-v2';
import { PaleteV2Toolbar } from '../components/palete-v2-toolbar';
import { SyncStatusV2ModalButton } from '../components/sync-status-v2-modal-button';
import { TemperaturaProdutoV2ModalButton } from '../components/temperatura-produto-v2-card';
import { useAdicionarItemV2 } from '../hooks/use-adicionar-item-v2';
...
interface ListaItensV2ViewProps {
  demandId: string;
}

export function ListaItensV2View({ demandId }: ListaItensV2ViewProps) {
  const navigate = useNavigate();
  const { process } = useProcessV2(demandId);
  const parametrosConferencia = useParametrosConferenciaV2(process?.unidadeId);
  const { getDivergencias, isLoading, removerItemAdicionado } = useConferenciaV2(demandId);
  const { checklist } = useChecklistV2(demandId);
  const adicionarItem = useAdicionarItemV2(demandId);
  ...
  async function handleRemoveAddedItem(item: DivergenciaItem) {
    const label = item.description?.trim() || item.sku;
    if (
      !confirm(
        `Excluir o item adicionado "${label}"?\n\nConferências e avarias deste SKU também serão removidas.`,
      )
    ) {
      return;
    }
    ...
    try {
      await removerItemAdicionado(item.sku);
      hapticMedium();
      toast.success('Item adicionado excluído');
    } catch (err) {
      ...
      <div className="sticky top-0 z-20 border-b border-outline-variant/60 bg-surface/95 backdrop-blur-md supports-[backdrop-filter]:bg-surface/80">
        <div className="flex items-center gap-3 px-margin-mobile py-3">
          ...
          <div className="flex items-center gap-1.5">
            <SyncStatusV2ModalButton demandId={demandId} />
            <Link
      ...
      <div className="mt-3 divide-y divide-outline-variant/50 px-margin-mobile">
        {isLoading ? (
          ...
          filteredItems.map((div) => {
            ...
            const isRemoving = removingSku === div.sku;
              ...
              >
                ...
                {div.isNovo ? (
                  <button
                    ...
                    disabled={isRemoving}
                    onClick={() => void handleRemoveAddedItem(div)}
                    className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-destructive touch-manipulation transition-transform active:scale-90 disabled:opacity-50"
                  >
                    {isRemoving ? (
