...
import { Route as DetalheItemRoute } from '@/routes/recebimento-v3/$id.index';

import { AdicionarProdutoSheetV2 } from '@/features/recebimento-v2/components/adicionar-produto-sheet-v2';
import { PaleteV2Toolbar } from '@/features/recebimento-v2/components/palete-v2-toolbar';
import { TemperaturaProdutoV2ModalButton } from '@/features/recebimento-v2/components/temperatura-produto-v2-card';
import { useAdicionarItemV2 } from '@/features/recebimento-v2/hooks/use-adicionar-item-v2';
...
import { useConferenciaV3 } from '../hooks/use-conferencia-v3';
...
interface ListaItensV3ViewProps {
  demandId: string;
}

export function ListaItensV3View({ demandId }: ListaItensV3ViewProps) {
  const navigate = useNavigate();
  const { process } = useProcessV2(demandId);
  const parametrosConferencia = useParametrosConferenciaV2(process?.unidadeId);
  const { getDivergencias, isLoading, removerItemAdicionado } = useConferenciaV3(demandId);
      ...
      <div className="sticky top-0 z-20 border-b border-outline-variant/60 bg-surface/95 backdrop-blur-md supports-[backdrop-filter]:bg-surface/80">
        <div className="flex items-center gap-3 px-margin-mobile py-3">
          ...
          <div className="flex items-center gap-1.5">
            <Link
              to="/recebimento-v3/$id/checklist"
              params={{ id: demandId }}
              search={{ view: '1' }}
              onClick={() => hapticLight()}
