...
import { DemandRcCard } from '../components/demand-rc-card';
import { RcOperadorSituacaoProvider } from '../context/rc-operador-situacao-context';
import { useListaDemandasRc } from '../hooks/use-lista-demandas-rc';
import { useProductCatalogSyncRc } from '../hooks/use-product-catalog-sync-rc';
import {
  matchesStatusFilter,
  type StatusFilter,
...
export function ListaDemandasRcView() {
  const {
    demandas,
    filteredDemandas,
    search,
  ...
  const { catalogError } = useProductCatalogSyncRc();
  ...
  const displayedDemandas = useMemo(
    () => filteredDemandas.filter((demanda) => matchesStatusFilter(demanda, statusFilter)),
    [filteredDemandas, statusFilter],
  );

  const demandCount = demandas.length;
    ...
    <PullToRefresh onRefresh={handleRefresh}>
      <div className="page-enter flex flex-col pb-safe-offset-4">
        ...
        <section className="flex flex-col gap-2 px-margin-mobile pb-2 pt-2">
          ...
          {isLoading ? (
          ...
          ) : missingUnidadeId || displayedDemandas.length === 0 ? (
            <EmptyState
            ...
            <RcOperadorSituacaoProvider value={operadorSituacaoById}>
              {displayedDemandas.map((demanda) => (
                <DemandRcCard key={demanda.preRecebimentoId} demanda={demanda} />
              ))}
            </RcOperadorSituacaoProvider>
          )}
        </section>
      </div>
    </PullToRefresh>
