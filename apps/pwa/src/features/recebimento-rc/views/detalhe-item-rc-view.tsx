...
import type { ConferenceRecord, ProductRecord } from '@/features/recebimento-v2/local-db/schema';
import {
  formatConferenceQuantityLabel,
  resolveConferenceQuantidadePar,
  resolveConferidoTotaisForSkuV2,
} from '@/features/recebimento-v2/lib/conferencia-quantidade';
import {
...
import { PaleteRcToolbar, type PaleteSheetIntent } from '../components/palete-rc-toolbar';
...
import {
  resolveProductForConferenciaRcAsync,
  useProcessLikeRc,
} from '../hooks/use-demanda-rc';
import { useParametrosConferenciaRc } from '../hooks/use-parametros-conferencia-rc';
import { useProductSearchRc } from '../hooks/use-product-search-rc';
import {
...
interface DetalheItemRcViewProps {
  demandId: string;
  sku?: string;
}
...
export function DetalheItemRcView({ demandId, sku: rawSku }: DetalheItemRcViewProps) {
  ...
  const [isResolvingInitialProduct, setIsResolvingInitialProduct] = useState(Boolean(initialSku));
  ...
  const { conferirItem, deletarConferencia, getConferenciasBySku, expectedItems } =
    useConferenciaRc(demandId);
  const { results: searchResults, isSearching } = useProductSearchRc(
    demandId,
    process?.unidadeId,
    searchQuery,
  );
  ...
  const schema = useMemo(
    () =>
      buildDetalheItemSchema({
        ...
        // v2: palete via PaleteRcToolbar (sessão), não campo idPalete no formulário
  ...
  useEffect(() => {
    if (!initialSku) {
      setIsResolvingInitialProduct(false);
      return;
    }

    let cancelled = false;
    setIsResolvingInitialProduct(true);

    void (async () => {
      const product = await resolveProductForConferenciaRcAsync(
        expectedItems,
        initialSku,
        process?.unidadeId,
      );
  ...
  }, [expectedItems, initialSku, process?.unidadeId, setValue]);

  async function selectProduct(product: ProductRecord) {
    hapticLight();
    const resolved = await resolveProductForConferenciaRcAsync(
      expectedItems,
      product.sku,
      process?.unidadeId,
    );
  ...
  const executeConferencia = useCallback(
    async (form: DetalheItemForm, focusAfter: PvarFocusAfterSubmit = 'gs1-peso') => {
      ...
      const resolvedProduct = await resolveProductForConferenciaRcAsync(
        expectedItems,
        form.sku,
        process?.unidadeId,
      );
  ...
  const submitConference = useCallback(
    async (form: DetalheItemForm, focusAfter: PvarFocusAfterSubmit = 'gs1-peso') => {
      ...
      if (isPvar || parametrosConferencia.controlaPalete) {
        const activePalete = getActivePaleteCodigoRc(demandId);
        if (!activePalete) {
  ...
  const handleConferenciaError = useCallback((err: unknown) => {
    ...
    if (message === PALETE_OBRIGATORIO_MSG_RC) {
  ...
  const handlePaleteConfirm = useCallback(
    async (codigo: string) => {
      const previous = getActivePaleteCodigoRc(demandId);
  ...
  async function handleDelete(conferenceId: string) {
    const record = conferences.find((entry) => entry.id === conferenceId);
    if (!record?.serverItemId?.trim() || record.serverItemId.startsWith('temp-')) {
      toast.warning('Aguarde a sincronização antes de remover');
      return;
    }
    ...
    if (
      !confirm(
        record.lote
          ? `Excluir conferência do lote ${record.lote}?`
          : 'Excluir esta conferência?',
      )
    ) {
      return;
    }
  ...
  return (
    <div className="page-enter flex flex-col pb-safe-offset-4">
      ...
      >
        {!product && isResolvingInitialProduct ? (
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            <Loader2 className="h-8 w-8 animate-spin text-secondary" aria-hidden />
            <p className="text-body-sm text-muted-foreground">Carregando produto...</p>
          </div>
        ) : null}
