...
import { TEMPERATURA_BAU_ETAPA_LABELS } from '@/features/recebimento-v2/lib/temperatura-bau-v2';

import {
  useTemperaturaRc,
  type TemperaturaEtapaRc,
  type TemperaturaEtapaStateRc,
} from '../hooks/use-temperatura-rc';

type TemperaturaFormValues = Record<TemperaturaEtapaRc, string>;
...
const INPUTS: Array<{ etapa: TemperaturaEtapaRc; label: string; hint: string }> = [
...
function buildFormFromEtapas(etapas: TemperaturaEtapaStateRc[]): TemperaturaFormValues {
  return {
...
export function TemperaturaRcModalButton({ demandId }: { demandId: string }) {
  const [open, setOpen] = useState(false);
  const [savingEtapa, setSavingEtapa] = useState<TemperaturaEtapaRc | null>(null);
  ...
  async function persistEtapa(etapa: TemperaturaEtapaRc) {
  ...
  async function handleBlur(etapa: TemperaturaEtapaRc) {
    hapticMedium();
  ...
  function handleKeyDown(
    ...
    etapa: TemperaturaEtapaRc,
    nextEtapa?: TemperaturaEtapaRc,
  ) {
    if (event.key !== 'Enter') return;
    event.preventDefault();
    void persistEtapa(etapa).then(() => {
      if (nextEtapa) {
        document.getElementById(`temp-rc-${nextEtapa}`)?.focus();
      }
    });
  }
  ...
  return (
    <>
      <button
        ...
        title={!canRegistrar ? 'Aguardando Replicache...' : undefined}
      >
      ...
      <Sheet open={open} onOpenChange={handleOpenChange}>
        ...
        >
          ...
          >
            ...
            {saveError ? <p className="text-label-sm text-destructive">{saveError}</p> : null}
          </div>
        </SheetContent>
      </Sheet>
    </>
