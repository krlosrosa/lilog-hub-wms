...
import type { SeparacaoPriority } from '../types/separacao.schema';

const PRIORITY_LABELS: Record<SeparacaoPriority, string> = {
  urgente: 'Urgente',
  normal: 'Normal',
};

interface SeparacaoOrderCardProps extends HTMLAttributes<HTMLButtonElement> {
  id: string;
  destino: string;
  zona: string;
  priority: SeparacaoPriority;
  itemCount: number;
  pickedCount: number;
  isPriority?: boolean;
  timeAgo?: string;
...
function MetaChip({
  ...
  tag,
  timeAgo,
}: Pick<SeparacaoOrderCardProps, 'isPriority' | 'tag' | 'timeAgo'>) {
  if (isPriority) {
    return (
...
export function SeparacaoOrderCard({
  id,
  destino,
  zona,
  priority,
  ...
  className,
  ...props
}: SeparacaoOrderCardProps) {
  const isUrgente = priority === 'urgente';
  const percent = itemCount > 0 ? Math.round((pickedCount / itemCount) * 100) : 0;
  const pending = Math.max(0, itemCount - pickedCount);
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0 flex-1 space-y-0.5">
          ...
          <p className="flex min-w-0 items-center gap-1 truncate text-body-sm text-on-surface-variant">
            <Truck className="h-3 w-3 shrink-0 text-secondary" aria-hidden />
      ...
      <div className="space-y-1 pl-[46px]">
        <div className="flex items-center justify-between text-[10px] text-on-surface-variant">
          <span>
            {pickedCount} separados · {pending} pendentes
          </span>
        ...
        <div
          ...
          aria-valuemin={0}
          aria-valuemax={100}
          aria-label={`Progresso da ordem ${id}`}
        >
          <div
